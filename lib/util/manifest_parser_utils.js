...
shaka.util.ManifestParserUtils = class {
  // AMAZON_MOD_BEGIN
  /**
   * Optimized resolution of base uril with relative url.  Relative url
   * is returned if it contains an absolute url.
   *
   * @param {string} baseUri
   * @param {string} relativeUri
   * @return {string}
   */
  static amzResolveUri(baseUri, relativeUri) {
    let resolvedUri;
    if (relativeUri.startsWith('http')) {
      resolvedUri = relativeUri;
    } else {
      resolvedUri = baseUri.slice(0, baseUri.lastIndexOf('/')) +
        '/' + relativeUri;
    }
    console.log('resovledUri = ', resolvedUri);
    return resolvedUri;
  }
  // AMAZON_MOD_END
  ...
  static resolveUris(baseUris, relativeUris) {
    ...
    if (baseUris.length == 1 && relativeUris.length == 1) {
      ...
      return [
        shaka.util.ManifestParserUtils
            .amzResolveUri(baseUris[0], relativeUris[0]),
      ];
      // AMAZON_MOD_END
    }
    ...
    for (const baseStr of baseUris) {
      ...
      for (const relative of relativeAsGoog) {
        resolvedUris.push(
            shaka.util.ManifestParserUtils.amzResolveUri(baseStr, relative));
      }
      // AMAZON_MOD_END
    }
