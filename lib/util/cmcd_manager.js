...
shaka.util.CmcdManager = class {
  ...
  appendSrcData(uri, mimeType) {
    try {
      ...
      const query = shaka.util.CmcdManager.toQuery(
          data, shaka.util.CmcdManager.getEncodeOptions_(uri));
  ...
  sendCmcdRequest_(cmcdData, target, response) {
    ...
    const encodeOptions = shaka.util.CmcdManager.getEncodeOptions_(baseURL);
  ...
  applyCmcdDataToRequest_(output, request, useHeaders) {
    const encodeOptions =
        shaka.util.CmcdManager.getEncodeOptions_(request.uris[0]);
    if (useHeaders) {
  ...
  static toHeaders(data, options) {
    ...
    for (let i = 0; i < headerGroups.length; i++) {
      const value =
          shaka.util.CmcdManager.serialize(headerGroups[i], options);
      if (value) {
  ...
  /**
   * Build encoder options for a given URI by deriving `baseUrl` from
   * its origin. Used by encode call sites so CML's `nor` formatter
   * produces root-relative URLs. Returns an empty options object if
   * the URI is missing, an `offline:` storage URI, or unparsable.
   *
   * @param {(string|undefined)} uri
   * @return {!cml.cmcd.CmcdEncodeOptions}
   * @private
   */
  static getEncodeOptions_(uri) {
    if (!uri || uri.includes('offline:')) {
      return /** @type {!cml.cmcd.CmcdEncodeOptions} */ ({});
    }
    try {
      return /** @type {!cml.cmcd.CmcdEncodeOptions} */ (
        {baseUrl: new URL(uri).origin});
    } catch (e) {
      return /** @type {!cml.cmcd.CmcdEncodeOptions} */ ({});
    }
  }
